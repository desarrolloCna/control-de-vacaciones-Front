...
import { registrarExcepcionLimite } from "../../../services/vacacionesespeciales/Vacacionesesepeciales.service";
import { PageHeader } from "../../../components/UI/UIComponents";
import api from "../../../config/api";

const ExcepcionLimitePage = () => {
    ...
    useEffect(() => {
        const fetchEmpleados = async () => {
            try {
                ...
                let data = response.data?.responseData?.emplloyeesList || [];
                
                // Filtrar solo a los activos
                data = data.filter(emp => emp.estado === "A" || emp.estado === "ACTIVO");
                
                // Filtrar por fecha de ingreso (< 2025) para que solo apliquen los antiguos
                ...
                const adaptedData = data.map(emp => ({
                    ...emp,
                    Nombre: emp.Nombres
                }));
                setEmpleadosU(adaptedData);
            } catch (error) {
                console.error("Error fetching empleados:", error);
                setMensaje({ tipo: 'error', texto: 'Error al cargar el listado de empleados.' });
            } finally {
        ...
        try {
            ...
            for (const empleado of empleadosProcesar) {
                try {
                    ...
                    const respuesta = await registrarExcepcionLimite(payload);
    ...
    return (
        <Box className="fade-in" sx={{ bgcolor: '#f4f6f8', minHeight: '100vh', pb: 4 }}>
            <PageHeader 
                title="Excepción de Límite de Vacaciones"
                subtitle="Personas que aplican a más de 20 días"
            />

            <Container maxWidth="md" sx={{ mt: 3 }}>
                {/* Banner informativo */}
                <Alert severity="info" sx={{ mb: 3, borderRadius: 2 }}>
                    <AlertTitle>Excepción de Límite de Vacaciones</AlertTitle>
                    El sistema bloquea por defecto solicitudes mayores a 20 días continuos. 
                    Esta herramienta permite autorizar temporalmente a empleados para que puedan 
                    solicitar todos los días que tengan acumulados.
                </Alert>
                                                    ...
                                                    {empleadosSeleccionados.has(empleado.idEmpleado) && (
                                                        <Box sx={{ mt: 2, width: '100%' }}>
                                                            <TextField
                                                                ...
                                                                multiline
                                                                rows={2}
                                                                label="Motivo para autorización"
                                                                variant="outlined"
                                                                size="small"
...
export default ExcepcionLimitePage;
