...
export const isHoliday = (date) => {
  const fullDate = dayjs(date).format("YYYY-MM-DD"); // Formato Fecha Completa (YYYY-MM-DD)
...
// Función combinada que calcula fecha fin, retorno laboral y extrae los días festivos cruzados
export const calcularRetornoYFestivos = (startDate, diasVacaciones) => {
  let fechaFin = dayjs(startDate);
  let diasContados = 0;
  let festivosEncontrados = [];

  const getHoliday = (date) => isHoliday(date);

  const initialHoliday = getHoliday(startDate);
  if (initialHoliday) {
    diasContados = 0; // El primer día oficial es festivo, no gasta saldo.
    // Opcional: festivosEncontrados.push(initialHoliday); 
    // pero usualmente el usuario no arranca sus vacaciones un asueto.
  } else {
    diasContados = 1;
  }
  ...
  while (diasContados < diasVacaciones) {
    ...
    const holidayDate = getHoliday(fechaFin);

    if (!isWeekend && !holidayDate) {
      diasContados++;
    } else if (holidayDate && !isWeekend) {
      if (!festivosEncontrados.find(f => f.idDiasFestivos === holidayDate.idDiasFestivos)) {
        festivosEncontrados.push(holidayDate);
      }
    }
  }

  // Ahora calcular la próxima fecha laboral de reintegro
  let proximaFecha = dayjs(fechaFin).add(1, "day");

  while (proximaFecha.day() === 0 || proximaFecha.day() === 6 || getHoliday(proximaFecha)) {
    const isWeekend = proximaFecha.day() === 0 || proximaFecha.day() === 6;
    const holidayDate = getHoliday(proximaFecha);
  ...
  return { fechaFin, proximaFechaLaboral: proximaFecha, festivosEncontrados };
};

// Nueva función: Validar que no se seleccione sábado o domingo ni festivos completos.
export const esDiaLaboral = (fecha) => {
  const dia = dayjs(fecha).day();
  if (dia === 0 || dia === 6) return false; // 0: Domingo, 6: Sábado
  const festivo = isHoliday(fecha);
  if (festivo && !festivo.medioDia) return false; // Evita que se escoga un festivo completo como inicio
  return true;
};
...
export const parseRecalculatedDates = (obs) => {
  if (!obs) return null;
  // Regex para atrapar fechas en formato DD/MM/YYYY globalmente
  const regex = /del (\d{2}\/\d{2}\/\d{4}) al (\d{2}\/\d{2}\/\d{4})/g;
  const matches = [...obs.matchAll(regex)];
  if (matches.length > 0) {
    // Tomar la fecha antigua del primer recalculo y la fecha nueva del último recalculo
    return {
      oldDate: matches[0][1],
      newDate: matches[matches.length - 1][2]
    };
  }
  return null;
};
